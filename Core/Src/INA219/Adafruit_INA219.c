...
extern I2C_HandleTypeDef _USER_DEFINE_I2C_INA219;

// The following multipliers are used to convert raw current and power => Các hệ số sau được sử dụng để chuyển đổi dòng điện thô và công suất
// values to mA and mW, taking into account the current config settings =>giá trị của mA và mW, có tính đến cài đặt cấu hình hiện tại
 ...
 * Used to avoid floating point numbers prolification */ // =>* Được sử dụng để tránh phân bổ số dấu phẩy động * /
int measureType;
...
int contMeasureInit(uint8_t reg)
{
	...
	HAL_I2C_Master_Transmit(&_USER_DEFINE_I2C_INA219, INA219_ADDRESS<<1, &reg, 1, 0xffffffff);
...
int contMeasureUpdate(void)
{
	...
	HAL_I2C_Master_Receive(&_USER_DEFINE_I2C_INA219, INA219_ADDRESS<<1, (uint8_t*)&measure, 2, 0xffffffff);
